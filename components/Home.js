...
function Home() {
  ...
  useEffect(() => {
    fetch(`https://my-moviz-backend-ten.vercel.app/movies`)
  ...
  const movies = movieData.map((movies, i) => {
 console.log(movies.poster_path)
   const isLiked = likedMovies.some(movie => movie === movies.title);
    ...
    isLiked={isLiked} 
    title={movies.title} 
    overview={movies.overview.length > 250 ?
      `${movies.overview.substring(0, 250)}...` : movies.overview
    }
